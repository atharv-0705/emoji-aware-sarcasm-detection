# ══════════════════════════════════════════════════════════════
#  Sarcoji — Combined Streamlit App (HF Spaces + Local)
#  ─────────────────────────────────────────────────────
#  This single file contains EVERYTHING needed to run the app:
#    - Model loading (from backend/model_loader.py)
#    - Prediction pipeline (from backend/predict.py)
#    - Streamlit UI (from frontend/app.py)
#    - Data loading (direct CSV reads from data/)
#
#  NO FastAPI, NO HTTP calls — runs as a single Streamlit process.
#
#  Usage:
#    streamlit run app.py
#
#  Works on:
#    - Hugging Face Spaces (free tier, CPU)
#    - Local machine (single-command launch)
# ══════════════════════════════════════════════════════════════
...
# ── Add backend/ to Python path so we can import its modules ──
PROJECT_ROOT = Path(__file__).resolve().parent
BACKEND_DIR  = PROJECT_ROOT / "backend"
sys.path.insert(0, str(BACKEND_DIR))

# ── Set TensorFlow env vars BEFORE any TF imports ─────────────
...
@st.cache_resource(show_spinner="🧠 Loading Sarcoji model... (first time only)")
def load_model():
    """Load the TensorFlow model and all artifacts once."""
...
def get_prediction(text: str) -> Dict[str, Any]:
    """Run prediction directly — no HTTP calls."""
    from predict import predict as run_predict
    return run_predict(text)


# ══════════════════════════════════════════════════════════════
# DATA LOADING (cached — reads CSVs directly)
# ══════════════════════════════════════════════════════════════
...
def get_correlation_stats() -> pd.DataFrame:
    """Read correlation stats from CSV."""
    try:
...
def get_emoji_data(top_n: int = 20) -> pd.DataFrame:
    """Read emoji analysis from CSV."""
    try:
        path = DATA_DIR / "emoji_analysis.csv"
        if path.exists():
            return pd.read_csv(path).head(top_n)
    except Exception:
        pass
    return pd.DataFrame()
...
COLORS = {
    "sarcastic"    : "#C62828",
    "non_sarcastic": "#1B5E20",
    "positive"     : "#1565C0",
    "negative"     : "#BF360C",
    "neutral"      : "#455A64",
    "bully"        : "#880E4F",
    "not_bully"    : "#1A237E",
    "confidence_hi": "#00695C",
    "confidence_lo": "#E65100",
    "highlight_max": "#FF1744",
    "highlight_mid": "#FF9100",
    "highlight_low": "#FFEA00",
}
...
with st.sidebar:
    ...
    if registry.is_ready:
        ...
        - **Vocab:** {len(registry.word2idx) if registry.word2idx else 0:,} tokens
        - **Threshold:** {registry.threshold:.4f}
        - **Max Len:** {cfg.get('max_len',50)} tokens
        """)
    else:
        st.error(f"❌ {registry.load_error or 'Not ready'}")

    st.divider()
    st.markdown("### 📌 Quick Links")
    st.markdown("""
    - [📁 GitHub](https://github.com/atharv-0705/emoji-aware-sarcasm-detection)
    - [🤗 HF Space](https://huggingface.co/spaces/athxrv/sarcoji-sentifusion-app)
    """)
...
with tab_predict:
    ...
    if predict_btn and user_input.strip():
        if not registry.is_ready:
            st.error(f"❌ Model not ready: {registry.load_error}")
        else:
            with st.spinner("🧠 Analysing..."):
                try:
                    result = get_prediction(user_input.strip())
...
with tab_analytics:
    ...
    if not emoji_df.empty and "Count" in emoji_df.columns:
        ...
        st.info("Emoji data not available. Check that emoji_analysis.csv is in data/")
...
with tab_model:
    ...
    if not metrics_df.empty:
        ...
        st.info("Model metrics not available. Add metrics_matrix.csv to data/")
